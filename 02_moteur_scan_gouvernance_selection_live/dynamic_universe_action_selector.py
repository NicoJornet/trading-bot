...
def single_score(row: pd.Series) -> float:
    return (
        ...
        + 0.20 * float(row.get("pair_memory_score", 0.0))
        + 1.50 * float(row.get("entry_timing_score", 0.0))
        + 1.20 * float(row.get("portfolio_diversification_score", 0.0))
        - 1.35 * float(row.get("portfolio_crowding_score", 0.0))
    )
...
def build_single_actions() -> pd.DataFrame:
    ...
    if "candidate" not in walk.columns:
        walk["candidate"] = pd.NA
    if "removed" not in walk.columns:
        walk["removed"] = pd.NA
    needs_parse = walk["candidate"].isna() | walk["removed"].isna()
    if needs_parse.any():
        parsed = walk.loc[needs_parse, "swap"].fillna("").astype(str).str.split("_for_", n=1, expand=True)
        if not parsed.empty and parsed.shape[1] == 2:
            walk.loc[needs_parse, "candidate"] = parsed[0].values
            walk.loc[needs_parse, "removed"] = parsed[1].values
    ...
    swap_memory = dug.load_swap_memory()
    if not swap_memory.empty:
        df = df.merge(
            ...
            ].drop_duplicates(["candidate", "removed"]),
            on=["candidate", "removed"],
            how="left",
        )
    db = read_optional_csv(DB_PATH)
    if not db.empty and "ticker" in db.columns:
        ...
        for col in candidate_meta_cols:
            if col not in db.columns:
                db[col] = 0.0 if col.endswith("_score") else ""
        candidate_meta = db[candidate_meta_cols].copy()
        candidate_meta = candidate_meta.rename(columns={"ticker": "candidate"})
        df = df.merge(candidate_meta, on="candidate", how="left")
    numeric_cols = [
        c
        for c in df.columns
        if c.endswith(("_pct", "_sharpe", "_ytd")) or "_wins_" in c or c.startswith("mean_delta_")
    ]
    ...
    for col in (
        ...
        if col not in df.columns:
            df[col] = 0.0
        df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0.0)
    if "pair_memory_label" not in df.columns:
        df["pair_memory_label"] = ""
    if "entry_timing_bucket" not in df.columns:
        df["entry_timing_bucket"] = ""
    df["selection_status"] = df.apply(classify_single, axis=1)
    df["selection_score"] = df.apply(single_score, axis=1)
    df = df.sort_values(
        ["selection_status", "selection_score", "portfolio_diversification_score", "pair_memory_score", "entry_timing_score", "oos_delta_roi_pct", "full_delta_roi_pct"],
...
def build_dynamic_add_actions() -> pd.DataFrame:
    ...
    for col in ("dynamic_conviction_score", "recent_score", "scan_algo_compat_score", "scan_algo_compat_score_v2", "profile_count", "entry_timing_score", "shadow_probation_score", "candidate_memory_score", "portfolio_crowding_score", "portfolio_diversification_score"):
        if col not in db.columns:
            db[col] = 0.0
        db[col] = pd.to_numeric(db[col], errors="coerce").fillna(0.0)
    if "entry_timing_bucket" not in db.columns:
        db["entry_timing_bucket"] = ""
    out = db.copy()
    ...
    out["selection_score"] = (
        out["selection_score"]
        + 1.50 * out["entry_timing_score"]
        + 0.50 * out["shadow_probation_score"]
        + 0.015 * out["candidate_memory_score"]
        + 1.20 * out["portfolio_diversification_score"]
        - 1.35 * out["portfolio_crowding_score"]
    )
    out = out.sort_values(
        ["selection_status", "selection_score", "portfolio_diversification_score", "entry_timing_score", "scan_algo_compat_score_v2", "recent_score", "scan_algo_compat_score"],
        ascending=[False, False, False, False, False, False, False],
    )
...
def choose_action_plan(
    ...
    for row in approved_singles.sort_values(
        ["selection_score", "portfolio_diversification_score", "pair_memory_score", "entry_timing_score", "mean_delta_sharpe_2017_2025", "mean_delta_roi_2017_2025"],
    ...
    for row in approved_adds.sort_values(
        ["selection_score", "portfolio_diversification_score", "entry_timing_score", "scan_algo_compat_score_v2", "recent_score", "scan_algo_compat_score"],
        ascending=[False, False, False, False, False, False],
    ).itertuples(index=False):
...
def write_outputs(single_df: pd.DataFrame, combo_df: pd.DataFrame, add_df: pd.DataFrame, remove_df: pd.DataFrame, selected: pd.DataFrame) -> None:
    ...
    duh.save_action_history(adds, dems, selected, decision_date=date.today().isoformat())
