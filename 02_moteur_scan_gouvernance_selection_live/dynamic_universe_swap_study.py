...
def aggregate_candidate_sources(base_prices) -> pd.DataFrame:
    ...
    dyn = pd.read_csv(dynamic_db_path, low_memory=False) if dynamic_db_path.exists() else pd.DataFrame(columns=["ticker"])
    ...
    hist_cols = [f"hist_{name}" for name in HIST_MERGE_FIELDS if f"hist_{name}" in merged.columns]
    if fill_map:
        fill_df = pd.DataFrame(fill_map, index=merged.index)
        merged = pd.concat([merged.drop(columns=hist_cols + list(fill_map.keys()), errors="ignore"), fill_df], axis=1)
    else:
        merged = merged.drop(columns=hist_cols, errors="ignore")
    ...
    if "promotion_stage" not in merged.columns:
        merged["promotion_stage"] = ""
    merged["promotion_stage"] = merged["promotion_stage"].fillna("")
    if "broker_tradeable" not in merged.columns:
        merged["broker_tradeable"] = True
    merged["broker_tradeable"] = merged["broker_tradeable"].fillna(True).astype(bool)
    ...
    merged = merged[
        ...
        & ((merged["promotion_stage"] != "blocked_broker") | allow_selected_adds)
        & (merged["broker_tradeable"] | allow_selected_adds)
    ].copy()
...
def main() -> None:
    ...
    swap_rows: list[dict] = []
    variant_prices_cache: dict[tuple[str, str], object] = {}
    ...
    for _pair_score, cand, rem in pair_queue:
        ...
        pair_key = (ticker, removed)
        variant_prices = variant_prices_cache.get(pair_key)
        if variant_prices is None:
            variant_prices = swap_prices(engine, base_prices, removed, ticker, candidate_map)
            variant_prices_cache[pair_key] = variant_prices
    ...
    for row in top_walk.itertuples(index=False):
        pair_key = (str(row.candidate), str(row.removed))
        variant_prices = variant_prices_cache.get(pair_key)
        if variant_prices is None:
