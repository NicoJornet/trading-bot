...
def main() -> None:
    ...
    active = pd.read_csv(ACTIVE_PATH, low_memory=False)["ticker"].dropna().astype(str).tolist()
    ...
    rows: list[dict] = []
    variant_prices_cache: dict[str, object] = {}
    ...
    for rem in demotions.itertuples(index=False):
        ...
        variant = variant_prices_cache.get(ticker)
        if variant is None:
            ...
            variant_prices_cache[ticker] = variant
